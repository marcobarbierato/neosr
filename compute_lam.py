...
from SaliencyModel.attributes import attr_grad, attr_id
...
class RSTBDense(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(2*dim, 2*dim, 3, 1, 1)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.residual_group2 = BasicLayer(
            dim=2*dim,
            input_resolution=input_resolution,
            depth=depth,
            num_heads=num_heads,
        ...
        if resi_connection == '1conv':
            ...
            self.conv = nn.Sequential(
                nn.Conv2d(dim, dim // 4, 3, 1,
                          1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(dim // 4, dim // 4, 1, 1,
                          0), nn.LeakyReLU(negative_slope=0.2, inplace=True),
        ...
        self.patch_embed2 = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=0, embed_dim=2*dim, norm_layer=None)
    ...
    def forward(self, x, x_size):
        z = self.patch_unembed2(torch.cat( (self.residual_group(x, x_size), x), 1), x_size)
        w = self.patch_embed2(self.lrelu(self.conv1(z)))
        y = self.patch_embed(self.conv(self.patch_unembed2(self.residual_group2(w, x_size), x_size)))
        return y
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            ...
            self.conv_after_body = nn.Sequential(
                nn.Conv2d(embed_dim, embed_dim // 4, 3, 1,
                          1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(embed_dim // 4, embed_dim // 4, 1, 1,
                          0), nn.LeakyReLU(negative_slope=0.2, inplace=True),
...
class ResidualDenseBlock(nn.Module):
    ...
    def __init__(self, num_feat=64, num_grow_ch=32):
        ...
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        #self.lrelu = nn.GELU()
        self.scale_res = nn.Parameter(torch.tensor(0.2))
        # initialization
...
class RRDB(nn.Module):
    ...
    def __init__(self, num_feat, num_grow_ch=32, attention=False):
    ...
    def forward(self, x):
        ...
        out = self.rdb3(out)
        # Empirically, we use 0.2 to scale the residual for better performance
        return self.attention(out) * self.scale_res + x

class pancamsr(nn.Module):

    def __init__(self, num_in_ch=3, num_out_ch=3, scale=upscale, num_feat=64, num_block=23, num_grow_ch=32, attention=False):
        super().__init__()
        self.scale = scale
        if scale == 2:
...
net_swim_pancam ={
  ...
  'depths': [6,6,6,6],
  'embed_dim': 42,
  'num_heads': [6,6,6,6], #num_head must divide embed_dim
...
model = pancamsr(**net_pancam).to(device='cuda')
#model = swinirdense(**net_swim_pancam).to(device='cuda')  

from copy import deepcopy
#load_net = torch.load('/home/mbarbier/superres/neosr/experiments/zzzSwinPancamFinal2/models/net_g_70000.pth', map_location=torch.device('cuda'))
load_net = torch.load('/home/mbarbier/superres/neosr/experiments/zzzCNNpancamFinal/models/net_g_80000.pth', map_location=torch.device('cuda'))
try:
...
from PIL import Image
img_lr = Image.open('/home/mbarbier/superres/val_linear/val3linear.png', mode='r')
tensor_lr=PIL2Tensor(img_lr)[2:, :, :]

tensor_lr = tensor_lr[:, :256, :256]

with torch.no_grad():
    hr=model(tensor_lr.unsqueeze(0).to(device='cuda'))

window_size = 16
w = 150  # The x coordinate of your select patch, 125 as an example
...
print('GINI')
print(100-100*gini(abs_normed_grad_numpy))
print('AOG')
print(np.sum(abs_normed_grad_numpy))
print('STD')
print(np.std(abs_normed_grad_numpy))
saliency_image_kde, cov = vis_saliency_kde(abs_normed_grad_numpy, zoomin=1)
make_pil_grid( [position_pil, cv2_to_pil(pil_to_cv2(saliency_image_kde) * (1.0 - alpha) + pil_to_cv2(position_pil) * alpha), Tensor2PIL(torch.clamp(result, min=0., max=1.), mode='L').resize((256,256))]).save('LAM/LAM_PANCAM_CNN.png')

#make_pil_grid( [position_pil, cv2_to_pil(pil_to_cv2(saliency_image_abs) * (1.0 - alpha) + pil_to_cv2(position_pil) * alpha), saliency_image_kde]).save('LAM/lam_swin_1.png')


#_, cov = vis_saliency_kde(abs_normed_grad_numpy, zoomin=1)
#print(cov)
